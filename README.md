...
props: {
      value: {// 1.1.1版本之后 新增v-model
        type: [Object, String],
        default: () => ('')
      },
      yhqContent: { // 1.1.1版本之后 不鼓励使用。可以直接使用 v-model
        type: String,
        default: () => ("")
      },
       yhqOptions: {
    ...
    1.1.1版本之后可以使用v-model双向绑定。（放弃原来的 yhqContent 传值）
...
#### Function:
  ...
  3、insertBlot(blot)：//自定义组件 插入静态变量变量标签  【1.1.1版本新增方法】
    参数 blot（类型obj）
    {
        title: "",
    ...
    4、blotContentReplace(content, str)  【1.1.1版本新增方法】
